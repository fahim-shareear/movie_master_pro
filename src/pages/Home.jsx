...
const Home = () => {
    ...
    const sortedByNewest = [...movies].sort((a, b) => new Date(b.addedAt || b._id) - new Date(a.addedAt || a._id));
    
    const heroMovies = sortedByNewest.slice(0, 4); 
    const topRatedMovies = [...movies].sort((a, b) => b.rating - a.rating).slice(0, 5); 
    const recentMovies = sortedByNewest.slice(0, 6); 
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-[#0F172A] text-slate-900 dark:text-white font-sans transition-colors duration-300">
                    ...
                    {heroMovies.map((movie) => (
                        <SwiperSlide key={movie._id}>
                            <div className="relative h-full w-full">
                                ...
                                <div className="absolute inset-0 bg-linear-to-r from-white via-white/90 dark:from-[#0F172A] dark:via-[#0F172A]/70 to-transparent flex items-center px-6 md:px-20">
                                    <motion.div initial={{ x: -50, opacity: 0 }} whileInView={{ x: 0, opacity: 1 }} className="max-w-2xl">
                                        ...
                                        <p className="text-slate-700 dark:text-white/70 text-lg font-medium dark:font-normal line-clamp-3 mb-8">{movie.details || movie.plotSummary}</p>
                                        <button 
                ...
                <div className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-8">
                    {[
                        ...
                        <div key={i} className="text-center">
                            ...
                            <p className="text-slate-500 dark:text-white/40 text-xs uppercase font-bold tracking-widest">{stat.label}</p>
                        </div>
                    ))}
                </div>
            </section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-5 gap-6">
                    {topRatedMovies.map((movie) => (
                        <motion.div 
                            ...
                            className="bg-white dark:bg-white/5 rounded-2xl overflow-hidden border border-slate-200 dark:border-white/10 group flex flex-col h-full shadow-sm dark:shadow-none"
                        >
                            <div className="relative overflow-hidden cursor-pointer" onClick={() => navigate(`/movie/${movie._id}`)}>
                                <img src={movie.posterImg} className="h-64 w-full object-cover group-hover:scale-110 transition-transform duration-500" alt={movie.title} />
                                <div className="absolute top-2 right-2 bg-slate-900/80 dark:bg-black/60 backdrop-blur-md px-2 py-1 rounded-lg text-[#EAB308] text-xs font-black">
                                    ★ {movie.rating}
                                </div>
                            </div>
                            <div className="p-4 text-center flex flex-col grow justify-between">
                                <h4 className="font-bold truncate text-sm mb-3 text-slate-900 dark:text-white group-hover:text-[#4F46E5] dark:group-hover:text-[#EAB308] transition-colors">{movie.title}</h4>
                                <button 
                                    onClick={() => navigate(`/movie/${movie._id}`)}
            ...
            {/* --- 4. RECENTLY ADDED --- */}
            <section className="bg-slate-50 dark:bg-white/5 py-20">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {recentMovies.map((movie) => (
                            <motion.div 
                                ...
                                className="bg-white dark:bg-[#1E293B] p-4 rounded-3xl flex gap-5 border border-slate-200 dark:border-white/5 hover:border-[#4F46E5]/50 transition-all group cursor-pointer shadow-md dark:shadow-none"
                                onClick={() => navigate(`/movie/${movie._id}`)}
                            >
                                <img src={movie.posterImg} className="w-24 h-32 object-cover rounded-xl shadow-lg" alt={movie.title} />
                                <div className="flex flex-col justify-between py-1">
                                    <div>
                                        <h3 className="font-black text-lg text-slate-900 dark:text-white group-hover:text-[#4F46E5] dark:group-hover:text-[#EAB308] transition-colors line-clamp-1">{movie.title}</h3>
                                        <p className="text-slate-500 dark:text-white/40 text-xs font-bold uppercase mt-1 tracking-widest">{movie.genre} • {movie.releaseDate?.split('-')[0] || 'N/A'}</p>
                                    </div>
                                    <button className="text-[#4F46E5] dark:text-[#EAB308] text-[10px] font-black tracking-widest uppercase flex items-center gap-2 hover:gap-3 transition-all cursor-pointer">
                                        View Details <HiOutlineArrowRight />
                                    </button>
                                </div>
                            </motion.div>
            ...
            {/* --- 5. GENRE SECTION (FIXED READABILITY) --- */}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {staticGenres.map((g) => (
                        <motion.div 
                            ...
                            className="bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 py-8 text-center rounded-2xl cursor-pointer font-black tracking-widest uppercase text-sm text-slate-900 dark:text-white shadow-sm dark:shadow-none transition-all"
                        >
                            {g}
                        </motion.div>
                    ))}
            ...
            <section className="bg-linear-to-t from-slate-100 to-transparent dark:from-black dark:to-transparent py-24 border-t border-slate-200 dark:border-white/5">
                <div className="max-w-5xl mx-auto px-6 text-center">
                    ...
                    <p className="text-slate-700 dark:text-white/60 text-lg font-medium dark:font-normal leading-relaxed mb-12 max-w-3xl mx-auto">
                    ...
                    <div className="flex flex-wrap justify-center gap-10 opacity-70 dark:opacity-60">
                        <span className="font-black text-xl italic tracking-tighter text-slate-500 dark:text-white/30">ULTRA-HD</span>
                        <span className="font-black text-xl italic tracking-tighter text-slate-500 dark:text-white/30">DTS-AUDIO</span>
                        <span className="font-black text-xl italic tracking-tighter text-slate-500 dark:text-white/30">PRO-UI</span>
                        <span className="font-black text-xl italic tracking-tighter text-slate-500 dark:text-white/30">DB-CLUSTERS</span>
                    </div>
                </div>
            </section>
        </div>
